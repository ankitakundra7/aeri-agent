"""AERI Scoring Agent — runs through Claude Code CLI.
...
This means AERI works if Paper2Agent works — no separate API key needed.
"""
...
AERI_PROMPT_TEMPLATE = """You are the AERI (Agent Executability And Readiness Index) scoring agent. You are evaluating 
...
def score_repository(repo_path: Path, structure_evidence: str,
    ...
    """Run the AERI agent via Claude Code CLI.
    ...
    prompt = AERI_PROMPT_TEMPLATE.format(
        ...
        for line in raw.splitlines():
            ...
            try:
                ...
                if obj.get("type") == "result":
                    ...
                    aeri_cost = obj.get("total_cost_usd", 0.0)
                    break
        ...
        if not response_text:
            ...
            try:
                ...
                if isinstance(outer, dict):
                    ...
                    aeri_cost = outer.get("total_cost_usd", 0.0)
            except json.JSONDecodeError:
                response_text = raw
        ...
        try:
            if isinstance(response_text, str):
                ...
                if start != -1 and end != -1:
                    ...
                    scores["aeri_cost_usd"] = round(aeri_cost, 4)
                    return scores
                else:
                    return {
                        ...
                        "classification": "Error",
                        "agent_ready": False,
                        "aeri_cost_usd": round(aeri_cost, 4),
                    }
            elif isinstance(response_text, dict):
                response_text["aeri_cost_usd"] = round(aeri_cost, 4)
