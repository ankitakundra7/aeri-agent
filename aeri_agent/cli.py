"""AERI Agent CLI — Score a repository's agent-readiness.

Uses Claude Code CLI (same as Paper2Agent) — no separate API key needed.

Usage:
    aeri-score https://github.com/owner/repo
    aeri-score https://github.com/owner/repo --abstract "Paper abstract..."
    aeri-score /path/to/local/repo --local
"""
...
from aeri_agent.repo_fetcher import clone_repo, RepoInfo
from aeri_agent.analyzers.structure import analyze_structure, format_evidence
from aeri_agent.analyzers.dependencies import analyze_dependencies, format_dep_evidence
from aeri_agent.agent import score_repository
from aeri_agent.report import to_markdown, to_json, format_score_bar

console = Console()


def print_score_table(scores: dict, name: str):
    table = Table(title=f"AERI Scores: {name}", show_header=True)
...
def main(repo_url, abstract, local, output, fmt, clone_dir, verbose, timeout):
    """Score a GitHub repository's agent-readiness using AERI.
    ...
    console.print(Panel.fit(
        "[bold blue]AERI Agent[/bold blue] — Agent Executability And Readiness Index Scorer\n"
    ...
    with Progress(SpinnerColumn(), TextColumn("[progress.description]{task.description}"),
        ...
        if local:
            ...
            target = Path(clone_dir) if clone_dir else Path(tempfile.mkdtemp(prefix="aeri_"))
        ...
        t4 = progress.add_task("AERI Agent scoring (Claude Code reading files)...", total=None)
        scores = score_repository(repo_info.local_path, se, de, abstract, timeout)
        progress.update(t4, description="✓ AERI Agent scoring complete")
    ...
    if "error" in scores:
        ...
        aeri_cost = scores.get("aeri_cost_usd", 0.0)
        if aeri_cost > 0:
            console.print(f"\n[bold]AERI Cost:[/bold] ${aeri_cost:.4f}")
            console.print(f"[dim]Paper2Agent average cost: $5-20 per conversion attempt[/dim]")
            console.print(f"[dim]Savings if not agent-ready: ${5 - aeri_cost:.2f} to ${20 - aeri_cost:.2f} per paper[/dim]")
    ...
    if output:
        p = Path(output)
        if fmt in ("json", "both"):
            p.with_suffix(".json").write_text(to_json(scores, repo_url, duration), encoding="utf-8")
            console.print(f"\n[green]JSON: {p.with_suffix('.json')}[/green]")
        if fmt in ("markdown", "both"):
            p.with_suffix(".md").write_text(to_markdown(scores, repo_url, duration), encoding="utf-8")
