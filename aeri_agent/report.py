...
def to_markdown(scores: dict, repo_url: str, duration: float) -> str:
    ...
    lines = [f"# AERI Report: {name}", f"**Repository:** {repo_url}",
    ...
    if aeri_cost > 0:
        lines.append(f"**AERI Scoring Cost: ${aeri_cost:.4f}**")
        lines.append(f"**Paper2Agent Cost (if run): $10–30**")
        if not ready:
            lines.append(f"**Estimated Savings: ${10 - aeri_cost:.2f} to ${30 - aeri_cost:.2f}**")
